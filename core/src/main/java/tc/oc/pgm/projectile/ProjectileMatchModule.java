...
public class ProjectileMatchModule implements MatchModule, Listener {
    ...
    private static final ThreadLocal<ProjectileDefinition> LAUNCHING_DEFINITION = new ThreadLocal<>();
    ...
    public void onClickEvent(PlayerInteractEvent event) {
        ...
        if (!isValidProjectileAction(event.getAction(), baseDefinition.clickAction()))
            return;
        ...
        try {
            assertTrue(LAUNCHING_DEFINITION.get() == null, "nested projectile launch");
            LAUNCHING_DEFINITION.set(definition);
            definition.launch(match.getPlayer(player), player.getEyeLocation());
        } finally {
            LAUNCHING_DEFINITION.remove();
        }

        if (baseDefinition.throwable()) {
            InventoryUtils.consumeItem(event);
        }

        if (baseDefinition.coolDown() != null) {
    ...
    public void onProjectileHurtEvent(EntityDamageByEntityEvent event) {
        ...
        if (projectileDefinition != null) {
            var baseDefinition = projectileDefinition.getBaseDefinition();
            if (!baseDefinition.potion().isEmpty()) {
                damagedEntity.addPotionEffects(baseDefinition.potion());
            }

            if (baseDefinition.damage() != null) {
                event.setDamage(baseDefinition.damage());
            }

            if (damagedEntity instanceof Player loser) {
                if (baseDefinition.onHitPlayerAction() != null) {
                    var mp = match.getPlayer(loser);
                    baseDefinition.onHitPlayerAction().trigger(mp);
                }
            }
        }
    }
    ...
    public void onProjectileHitEvent(ProjectileHitEvent event) {
        ...
        var baseDefinition = projectileDefinition.getBaseDefinition();
        Filter filter = baseDefinition.destroyFilter();
        ...
        if (hitBlock != null) {
            ...
            if (filter.query(query).isAllowed()) {
                ...
                if (baseDefinition.onHitBlockAction() != null) {
                    baseDefinition.onHitBlockAction().trigger(new MatchLocation(match, hitBlock.getLocation()));
                }
            }
        }
    }
    ...
    public ProjectileDefinition getProjectileDefinition(String projectileId) {
        ...
        for (ProjectileDefinition projectileDefinition : projectileDefinitions) {
            if (projectileId.equals(projectileDefinition.getBaseDefinition().id())) {
    ...
    public static @Nullable ProjectileDefinition getProjectileDefinition(Entity entity) {
        ...
        return LAUNCHING_DEFINITION.get();
    }
